...
object WikipediaRanking {
   ...
   *   several seconds.
   */
  def compare(lt: ((String, Int), (String, Int))) = { lt._1._2 > lt._2._2 }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    ...
    langs.map(lang => makeIndex(lang, rdd))
      .reduce(_ union _)
  }

  private def makeIndex(lang: String, rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.map((lang, _))
